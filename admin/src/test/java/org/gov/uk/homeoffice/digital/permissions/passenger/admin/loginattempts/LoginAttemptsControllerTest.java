...
import static org.gov.uk.homeoffice.digital.permissions.passenger.admin.loginattempts.LoginAttemptsController.FORMATTER;
...
public class LoginAttemptsControllerTest {
    ...
    public void loginAttempts() {
        ...
        verify(loginAttemptsService, times(1)).allLoginAttemptsBetween(parse(FROM, FORMATTER), parse(TO, FORMATTER));
    ...
    public void loginAttemptsWhenUserHasMultipleLoginAttempts() {
        ...
        when(loginAttemptsService.allLoginAttemptsBetween(parse(FROM, FORMATTER), parse(TO, FORMATTER))).thenReturn(loginAttempts);
